...
<body>
				...
				<ul class="nav" id="side-menu">
					<%-- <li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							...
							<li><a href="<%=basePath%>mana/addgoodsrecycle"  class="active">商品回收站</a></li>
								...
								<table width="100%"
									...
									<tbody style="cursor: pointer;">
										 <c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												<td onclick="clicklist(${mi.id},${mi.kind})">${mi.name}</td>
												<%-- <td onclick="clicklist(${mi.id})"><img alt=""
													src="${mi.pic}" style="width:40px; height:40px"></td> --%>
													<td onclick="clicklist(${mi.id},${mi.kind})">${mi.standard}</td>
													<td onclick="clicklist(${mi.id},${mi.kind})">${mi.count}份</td>
												<td onclick="clicklist(${mi.id},${mi.kind})">${mi.price}</td>
												<td onclick="clicklist(${mi.id},${mi.kind})">${mi.categroy }</td>
												<td onclick="comment(${mi.id},'${mi.name}')">查看评论</td>
												<td onclick="dellist(${mi.id})"><i
													class="glyphicon glyphicon-repeat"></i></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
	...
	<script>
		...
		function clicklist(i,kind) {
			//alert("进入" + i)
			window.location.href = "<%=basePath%>mana/addgoods?id=" + i+"&kind="+kind;
		}
		function comment(i,name) {
			window.location.href = "<%=basePath%>mana/commentlist?id=" + i+"&name="+name;
		}
		function dellist(i) {
			var con = confirm("确定重新上架商品吗？")
			if (con == true) {
				//alert("确定")
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/recyclegoods",
							data : {
								"id" : i
							},
