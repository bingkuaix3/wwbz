...
<body>
	<div id="wrapper">
			...
			<div class="sidebar-nav navbar-collapse">
				<ul class="nav" id="side-menu">
					<%-- <li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							<li><a href="<%=basePath%>mana/articlelistgl" class="active">文章列表</a></li>
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">管理端小程序首页文章列表</h1>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
										 ...
										 <c:forEach var="mi" items="${requestScope.al}">
											<tr class="odd gradeX">
												...
												<td onclick="clicklist(${mi.id})"><img alt=""
													src="${mi.img}" style="width: 40px; height: 40px;"></td>
												<td onclick="clicklist(${mi.id})"><c:choose>
														<c:when test="${ mi.isday== 0}">
															否
														</c:when>
														<c:otherwise>
															是
	...
	<script>
		...
		function clicklist(i) {
			//alert("进入" + i)
			window.location.href = "<%=basePath%>mana/addarticlegl?id=" + i;
		}
		function dellist(i) {
			...
			if (con == true) {
				...
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/delarticlegl",
							data : {
								"id" : i
							},
