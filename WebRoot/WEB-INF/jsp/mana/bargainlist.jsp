...
<body>
	<div id="wrapper">
			...
			<div class="sidebar-nav navbar-collapse">
				<ul class="nav" id="side-menu">
					...
					<li><a href="#"><i class="glyphicon glyphicon-copyright-mark"></i>
						...
						<ul class="nav nav-second-level">
							<li><a href="<%=basePath%>mana/bargainlist" class="active">砍价活动列表</a></li>
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">砍价活动列表</h1>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
									...
									<tbody style="cursor: pointer;">
										 <c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												...
												<td onclick="clicklist(${mi.id})">${mi.percent}</td>
												<td onclick="clicklist(${mi.id})">${mi.endtime}</td>
	...
	<script>
		...
		function clicklist(i) {
			//alert("进入" + i)
			window.location.href = "<%=basePath%>mana/addbargain?id=" + i;
		}
		function dellist(i) {
			var con = confirm("确定删除活动吗？")
			if (con == true) {
				//alert("确定")
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/delbargain",
							data : {
								"id" : i
							},
