				...
				<ul class="nav" id="side-menu">
					<%-- <li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							...
							<li><a href="<%=basePath%>mana/deallist" class="active">已处理订单</a></li>
						</ul> <!-- /.nav-second-level --></li>
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">订单列表<button class="btn btn-primary" id="b" style="margin-left:10px;">导出excel</button></div>
							<!-- /.panel-heading -->
									...
									<tbody >
										<c:forEach var="mi" items="${requestScope.list}">
											<c:choose>
												...
												<c:otherwise>
													<tr class="odd gradeX">
														<td onclick="clicklist(${mi.id})">${mi.goodsname}</td>
														<td onclick="clicklist(${mi.id})"><fmt:formatDate
																value="${mi.buytime}" pattern="yyyy/MM/dd HH:mm:ss" /></td>
														<td onclick="clicklist(${mi.id})">${mi.number}</td>
														<td><c:choose>
																<c:when test="${mi.state==2}">
														已发货
													</c:when>
																<c:otherwise>
															已收货
														...
														<td onclick="clicklist(${mi.id})">${mi.money}</td>
													</tr>
												</c:otherwise>
											</c:choose>
												...
												<c:otherwise>
													<tr class="odd gradeX">
														...
														正常商品
															...
															<td>${mi.name}</td>
															<td>${mi.tel}</td>
															<td>${mi.sheng}&nbsp&nbsp${mi.shi}&nbsp&nbsp${mi.qu}</td>
															<td>${mi.address}</td>
															<td>${mi.lw}</td>
															<td>${mi.com}</td>
															<td>${mi.num}</td>
	...
	<script>
		$(document).ready(function() {
			...
			$("#b").click(function () {
				...
				var name=new Date().toLocaleString().substr(0,10)
				
		         $("#all").table2excel({
		            exclude: "", //过滤位置的 css 类名  
		            filename: name+".xls", //文件名称  
		...
		function clicklist(i) {
			//alert("进入" + i)
			window.location.href = "<%=basePath%>mana/order?id=" + i;
		}
		function dellist(i) {
