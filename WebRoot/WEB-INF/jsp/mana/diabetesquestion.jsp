											...
											<c:forEach var="mi" items="${requestScope.qlist}">
												<div>
														...
														<c:choose>
															<c:when test="${mis.parentid==mi.id}">
																<div style="display: flex; flex-direction: row; just-content: flex-start; align-items: center;margin-bottom: 1%;">
																	<input type="hidden" class="form-control" value="${mis.parentid }">
																	<input type="text" class="form-control" value="${mis.content }" style="width:60%;">
																	<input type="hidden" class="form-control" value="${mis.id }">
																		<c:choose>
																			<c:when test="${mis.score!=0}">
																				<input type="checkbox" checked style="margin-left:10px" >正确选项
																			</c:when>
																			<c:otherwise>
																				<input type="checkbox" style="margin-left:10px">正确选项
         																	</c:otherwise>
																		</c:choose>
													...
													<div class="from-control son"
														...
														<input type="text" class="form-control" placeholder="请输入题目" style="width:80%;"><input type="checkbox" style="margin-left:10px">正确选项
			...
			<script>
				$(document).ready(function() {
					...
					$(".change").click(function(){
						if($(this).prev().prev().prev().val()==""){
						 ...
						 if(a-parseInt(a)==0){
							 $.ajax({
									type : "POST",
									url : "<%=basePath%>mana/adddiabetesquestion",
											data : {
					...
					$(".del").click(function(){
						$.ajax({
							type : "POST",
							url : "<%=basePath%>mana/deldiabetesquestion",
									data : {
					...
					$(".addson").click(function(){
						 if($(this).prev().val()==""){
							...
							$.ajax({
								type : "POST",
								url : "<%=basePath%>mana/adddiabetesquestionson",
										data : {
											"fatherid":$(this).parent().parent().find(".id").val(),
											"content":$(this).prev().prev().val(),
											"right":($(this).prev().is(':checked'))?1:0,
											"id":0
										},
										dataType : 'json',//返回值是json
										async : false,//同步请求
					...
					$(".changeson").click(function(){
						if($(this).prev().prev().val()==""){
							...
							$.ajax({
								...
								url : "<%=basePath%>mana/adddiabetesquestionson",
										data : {
											"fatherid":$(this).prev().prev().prev().prev().val(),
											"content":$(this).prev().prev().prev().val(),
											"id":$(this).prev().prev().val(),
											"right":$(this).prev().is(':checked')?1:0
										},
