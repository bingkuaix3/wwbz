...
<body>
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="panel panel-default">
							<div class="panel-heading">限时活动列表</div>
							<!-- /.panel-heading -->
									...
									<tbody style="cursor: pointer;">
										<c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												...
												<td onclick="clicklist(${mi.id})"><fmt:formatDate value="${mi.stime}" pattern="yyyy/MM/dd HH:mm:ss" /></td>
												<td onclick="clicklist(${mi.id})"><c:choose>
														<c:when test="${ mi.state==0 }">
															筹备中(可以修改)
														</c:when>
														<c:when test="${ mi.state==1 }">
															倒计时(不可修改)
														</c:when>
														<c:when test="${ mi.state==2 }">
															进行中(不可修改)
														</c:when>
														<c:when test="${ mi.state==3 }">
															已结束(不可修改)
														</c:when>
													</c:choose></td>
												<td onclick="dellist(${mi.id})"><i class="fa fa-trash-o fa-fw"></i></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
	...
	<script>
		...
		function clicklist(i) {
			//alert("进入" + i)
			window.location.href = "<%=basePath%>mana/addmission?id=" + i;
		}
		function dellist(i) {
