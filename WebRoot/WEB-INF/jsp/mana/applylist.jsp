...
<body>
	<div id="wrapper">
				...
				<ul class="nav" id="side-menu">
					<%-- <li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							...
							<li><a href="<%=basePath%>mana/applylist" class="active">经销商申请列表</a></li>
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">经销商申请列表</h1>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
									...
									<tbody style="cursor: pointer;">
										<c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												...
												<td onclick="clicklist(${mi.id})">${mi.pay}元</td>
												<td onclick="clicklist(${mi.id})">${mi.forecast}万元</td>
												<td onclick="del(${mi.id})"><i
 ...
 --%>
		...
		<%-- function clicklist(i) {
			window.location.href = "<%=basePath%>mana/pass?id=" + i;
		} --%>
		function del(i) {
			var con = confirm("确认删除此条申请？")
			if (con == true) {
				//alert("确定")
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/delapplyx",
							data : {
								"id" : i
							},
		...
		function clicklist(i){
			window.location.href = "<%=basePath%>mana/applycontent?id=" + i+"&state=0";
		}
