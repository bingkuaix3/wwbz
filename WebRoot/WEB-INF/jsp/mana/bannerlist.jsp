...
<body>
			...
			<div class="sidebar-nav navbar-collapse">
				<ul class="nav" id="side-menu">
					<%-- <li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							<li><a href="<%=basePath%>mana/bannerlist"  class="active">轮播列表</a></li>
								...
								<table width="100%"
									...
									<tbody style="cursor: pointer;">
										 <c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												<td onclick="clicklist(${mi.id})">${mi.index}</td>
												<td onclick="clicklist(${mi.id})">${mi.name}</td>
												<td onclick="clicklist(${mi.id})">
												<img alt="" src="${mi.url}" style="width:32px;height:24px">
												</td>
												<td onclick="clicklist(${mi.id})"><c:choose>
														<c:when test="${mi.kind==0}">
														小程序页面
													</c:when>
													<c:when test="${mi.kind==1}">
														公众号文章
													</c:when>
													<c:when test="${mi.kind==2}">
														新页面（图片）
													</c:when>
														<c:otherwise>
															新页面（编辑页面）
	...
	<script>
		...
		function clicklist(i) {
			//alert("进入" + i)
			window.location.href = "<%=basePath%>mana/addbanner?id=" + i;
		}
		function dellist(i) {
			var con = confirm("确定删除轮播吗？")
			if (con == true) {
				//alert("确定")
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/delbanner",
							data : {
								"id" : i
							},
