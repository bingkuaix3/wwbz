	...
	<div id="wrapper">
				...
				<ul class="nav" id="side-menu">
					<%-- <li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							...
							<li><a href="<%=basePath%>mana/rewardlist" class="active">积分增减</a></li>
						</ul> <!-- /.nav-second-level --></li>
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">用户积分列表</h1>
					</div>
					<!-- /.col-lg-12 -->
				</div>
				<!-- /.row -->
									...
									<tbody style="cursor: pointer;">
										 <c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												<td>${mi.nickname}</td>
												<td>${mi.reward}</td>
												<td onclick="change(${mi.id},'${mi.nickname}')"><i
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header">
				...
				<div class="modal-body">
					<div>
						<label id="name">变更人昵称:</label>
						<input type="hidden" id="id">
					</div>

					<div>
						<label>变更积分</label> <input class="form-control" type="number" id="reward">
					</div>
					<div>
						<label>变更说明</label> <input class="form-control" id="reason">
					</div>
				</div>
				<div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" class="btn btn-primary" onclick="con()">确定</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
</div>
	...
	<script>
		...
		function change(i,name) {
			$("#modal").click()
			$("#name").html("变更人昵称:"+name)
			$("#id").val(i)
		...
		function con(){
			if($("#reward").val()==""||$("#reward").val()==0||$("#reason").val()==""){
				alert("请补全信息")
			}else{
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/changereward",
							data : {
								"id" : $("#id").val(),
								"reward":$("#reward").val(),
								"reason":$("#reason").val()
