...
<body>
									...
									<tbody >
										<c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												...
												<td onclick="clicklist(${mi.id})">${mi.city }</td>
												<td onclick="clicklist(${mi.id})">${mi.kind }</td>
												<c:choose>
													<c:when test="${mi.isshow==1}">
														<td onclick="clicklist(${mi.id})">是</td>
													</c:when>
													<c:otherwise>
														<td onclick="clicklist(${mi.id})">否</td>
													</c:otherwise>
												</c:choose>
												<td onclick="clicklist(${mi.id})">${mi.tel }</td>
												<td onclick="allot(${mi.id})"><i class="glyphicon glyphicon-edit"></i></td>
											</tr>
										</c:forEach>
									</tbody> 
								</table>
	...
	<script>
		...
		function dellist(i) {
			var con = confirm("确定取消合作吗？")
			if (con == true) {
				//alert("确定")
				$.ajax({
					type : "POST",
					url : "<%=basePath%>mana/delapply",
							data : {
								"id" : i
							},
		...
		function clicklist(i){
			window.location.href = "<%=basePath%>mana/applycontent?id=" + i+"&state=1";
		}
		function allot(i){
			window.location.href = "<%=basePath%>mana/dealerallot?id=" + i ;
		}
