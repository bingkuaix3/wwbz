						...
						<div class="col-lg-12">
							<div class="panel panel-default">
								...
								<div class="panel-body">
									<div class="row">
										<div class="col-lg-6">
											<form role="form" action="<%=basePath%>mana/changeorder"
												method="post" 
												onsubmit="return validate_required()">
												<div class="form-group">
													<label>商品名称</label>
													<div>${requestScope.record.goodsname }</div> 
													<input type="hidden" name="record_id" id="record_id"
														value="${requestScope.record.id }">
												</div>
												<div class="form-group">
													<label>消费数量</label> 
													<div>${requestScope.record.number }</div> 
												</div>
												<div class="form-group">
													<label>消费价格</label> 
													<div>${requestScope.record.money }</div> 
												</div>
												<div class="form-group">
													<label>商品种类</label>
													<div>
														<c:choose>
															<c:when test="${requestScope.record.kind==0}">
														正常商品
													</c:when>
															<c:otherwise>
															积分商品
												...
												<div class="form-group">
													<label>收货人</label> 
													<div>${requestScope.record.name }</div> 
												</div>
												<div class="form-group">
													<label>联系电话</label>
													<div>${requestScope.record.tel }</div> 
												</div>
												<div class="form-group">
													<label>地区</label>
													<div>${requestScope.record.sheng }&nbsp&nbsp${requestScope.record.shi }&nbsp&nbsp${requestScope.record.qu }</div> 
												</div>
												<div class="form-group">
													<label>详细地址</label>
													<div>${requestScope.record.address }</div> 
												</div>
												<div class="form-group">
													<label>订单收益</label>
													<div>邀请人：${requestScope.father }&nbsp&nbsp&nbsp&nbsp大区收益人：${requestScope.head }</div> 
												</div>
												<div class="form-group">
													<label>用户留言</label>
													
													<c:choose>
															<c:when test="${requestScope.record.lw==''}">
														<div>无</div> 
													</c:when>
															<c:otherwise>
															<div>${requestScope.record.lw }</div> 
													</c:otherwise>
														</c:choose>
												</div>
												<div class="form-group">
													...
													<select class="form-control" name="com" id="com" >
														...
														<option value="youzhengguonei">邮政国内/挂号信</option>
														<option value="youzhengguonei">EMS/E邮宝</option>
													</select>
												</div>
												<div class="form-group">
													<label>快递单号</label>
													<input class="form-control" name="num" id=num value="${requestScope.record.num }">
												</div>
