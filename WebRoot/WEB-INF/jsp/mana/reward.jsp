...
<%-- 					<li><a href="#"><i class="glyphicon glyphicon-time"></i>
						...
						<ul class="nav nav-second-level">
							<li><a href="<%=basePath%>mana/reward"  class="active">积分系数</a></li>
		...
		<div id="page-wrapper">
						...
						<div class="col-lg-12">
							<div class="panel panel-default">
								...
								<div class="panel-body">
									<div class="row">
										<div class="col-lg-6">
											<form role="form" action="<%=basePath%>mana/savereward"
												method="post" enctype="multipart/form-data"
												onsubmit="return validate_required()">
												<div class="form-group">
												<label>购买商品积分系数</label>
													<input class="form-control" name="goods" id="goods" value="${requestScope.re.goods }" type="number"  step="0.1">
												</div>
												<div class="form-group">
												<label>建立档案奖励积分</label>
													<input class="form-control" name="archives" id="archives" value="${requestScope.re.archives }" type="number">
												</div>
												<div class="form-group">
												<label>分享档案奖励积分</label>
													<input class="form-control" name="share" id="share" value="${requestScope.re.share }" type="number">
												</div>
												<div class="form-group">
												<label>添加每日血糖奖励积分</label>
													<input class="form-control" name="day" id="day" value="${requestScope.re.day }" type="number">
												</div>
												<div class="form-group">
												<label>老用户最低摇一摇最低积分</label>
													<input class="form-control" name="oldlow" id="oldlow" value="${requestScope.re.oldlow }" type="number">
												</div>
												<div class="form-group">
												<label>老用户最低摇一摇最高积分</label>
													<input class="form-control" name="oldhigh" id="oldhigh" value="${requestScope.re.oldhigh }" type="number">
												</div>
												<div class="form-group">
												<label>新用户最低摇一摇最低积分</label>
													<input class="form-control" name="newlow" id="newlow" value="${requestScope.re.newlow }" type="number">
												</div>
												<div class="form-group">
												<label>新用户最低摇一摇最高积分</label>
													<input class="form-control" name="newhigh" id="newhigh" value="${requestScope.re.newhigh }" type="number">
												</div>
												<div class="form-group">
												<label>转发标题</label>
													<input class="form-control" name="sharetitle" id="sharetitle" value="${requestScope.re.sharetitle }">
												</div>
												<div class="form-group">
												<label>转发图片</label> <input type="file"
														name="shareimg" id="shareimg"
														onchange="chang(event)"> <img alt="再无图片"
														id="myLimg" src="${requestScope.re.shareimg}" height="100px"
														style="margin-top: 2%">
												</div>
			...
			<script>
				...
				function validate_required() {
					if($("#goods").val()==""||$("#archives").val()==""||$("#share").val()==""||$("#day").val()==""||$("#oldlow").val()==""||$("#oldhigh").val()==""||$("#newlow").val()==""||$("#newhigh").val()==""||$("#sharetitle").val()==""){
						alert("请补全信息")
						return false;
					}else{
						//alert($("#oldhigh").val()<=$("#oldlow").val())
						if($("#oldhigh").val()<=$("#oldlow").val()||$("#newhigh").val()<=$("#newlow").val()){
							alert("请检查信息")
							return false;
						}else{
							return true;
