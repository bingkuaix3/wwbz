...
<body>
				...
				<ul class="nav" id="side-menu">
					...
					<li><a href="#"><i class="glyphicon glyphicon-qrcode"></i>
						...
						<ul class="nav nav-second-level">
							...
							<li><a href="<%=basePath%>mana/invite" class="active">经销商邀请关系（修改）</a></li>
			...
			<div class="container-fluid">
				...
				<div class="row">
									...
									<tbody style="cursor: pointer;">
										<c:forEach var="mi" items="${requestScope.list}">
											<tr class="odd gradeX">
												...
												<td onclick="change('${mi.openid}','${mi.name}')"><i class="	glyphicon glyphicon-edit"></i></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
					...
					<div class="col-lg-6" id="list">
						<div class="form-group">经销商列表</div>
						<c:forEach var="mi" items="${requestScope.dealerlist}">
							<div onclick="invite('${mi.openid}','${mi.name}')">${mi.name }<i class="glyphicon glyphicon-ok"></i></div>
						</c:forEach>
					</div>
				</div>
				<!-- /.row -->
	...
	<script>
				...
				function invite(openids,names){
					//alert(name)
					var con = confirm("确定将"+names+"绑定为"+name+"的上级么？")
					if (con == true) {
						$.ajax({
							type : "POST",
							url : "<%=basePath%>mana/changeinvite",
									data : {
										"openid" : openid,
										"fopenid" :openids
									},
